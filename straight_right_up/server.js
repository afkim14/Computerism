...
io.on('connection', function(socket) {
    ...
    socket.on('circle_edge', function(data) {
      if (socket.id == panel_ids[0]) {
        SendArtwork(1, 0, radius, colors[0], getStartingPos(1,0), getDestination(1,0));
...
function getDestination(panelIndex, option) {
  ...
  if (option == 0) {
    ...
    destination = {
      emit_destination: [constrainedCanvasSizes[panelIndex].width/2, 0 + radius],
      dead_destination: [constrainedCanvasSizes[panelIndex].width/2, 0 - radius],
      tolerance: 0.5
    };
  }
  return destination;
