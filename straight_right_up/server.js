...
io.on('connection', function(socket) {
    ...
    socket.on('circle_edge', function(data) {
      if (socket.id == panel_ids[0]) {
        SendArtwork(1, 0, radius, circleColor1, startingPos1, destination1);
...
function InitiateArtwork() {
  ...
  destination2 = {
    emit_destination: [constrainedCanvasHeight/2, 0 + radius],
    dead_destination: [constrainedCanvasHeight/2, 0 - radius],
    tolerance: 0.5
  };
  startingPos1 = {
    x: 0-radius,
